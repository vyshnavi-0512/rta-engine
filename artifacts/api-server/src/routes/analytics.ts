...
router.post("/events", async (req, res): Promise<void> => {
  ...
  broadcastWsMessage({ type: "event", data: event });
...
router.get("/stats", async (req, res): Promise<void> => {
  ...
  const totalPageViews = allRows.filter((r) => r.eventType === "page_view").length;
  const totalErrors = allRows.filter((r) => r.eventType === "error").length;
  const totalClicks = allRows.filter((r) => r.eventType === "button_click").length;
  const activeUsers = new Set(recentRows.map((r) => r.userId)).size;
  const errorRate = totalEvents > 0 ? (totalErrors / totalEvents) * 100 : 0;

  const responseTimes = allRows
    .filter((r) => r.metadata && typeof (r.metadata as Record<string, unknown>)["responseMs"] === "number")
    .map((r) => (r.metadata as Record<string, unknown>)["responseMs"] as number);
  const avgResponseMs =
...
router.get("/active-users", async (req, res): Promise<void> => {
  ...
  const current = new Set(currentRows.map((r) => r.userId)).size;
...
router.get("/error-rate", async (req, res): Promise<void> => {
  ...
  const currentErrors = currentRows.filter((r) => r.eventType === "error").length;
  const currentRate = currentRows.length > 0 ? (currentErrors / currentRows.length) * 100 : 0;
  const prevErrors = prevRows.filter((r) => r.eventType === "error").length;
  const prevRate = prevRows.length > 0 ? (prevErrors / prevRows.length) * 100 : 0;
